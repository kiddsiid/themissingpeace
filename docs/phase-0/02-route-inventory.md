...
**Purpose:** enumerate every current route in the local Next.js app, map it to the directive's module list, and mark status + gaps. Routes are from `app/**` in the local project (mirrored in `_reference_nextjs/app`).
...
4. Feast Studio — ❌ **missing in app** (exists in Emergent CRA + prototype HTML)
5. Atmosphere Lab — ❌ **missing in app** (exists in Emergent CRA + prototype HTML)
