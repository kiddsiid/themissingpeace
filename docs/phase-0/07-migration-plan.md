...
**Purpose:** the ordered, gated path from today's reality (a strong Next.js engine plus a standalone `localStorage` prototype) to the directive's end state (one hardened engine, two role-specific experiences, a truthful seeded demo, and a cinematic public site). Mapped to the directive's Phases 1–8. Every phase ends with the directive's nine-part report and stops at its gate.

**Governing principle:** *preserve and harden, do not rebuild.* Migration is additive-plus-hardening on the canonical Next.js product.
...
- **A-0 One repo of record.** Keep the Next.js project in a single canonical repository and working path.
...
- **Repo hygiene.** Untrack build artifacts/zips; delete temporary prototype working folders; consolidate deploy configs to one product + one demo target.
...
- **Feast Studio**: menus/courses/dishes + normalized dietary requirements + caterer brief + guest plate preview + guest-care warnings + advisory cost guidance, built against the canonical schema.
